...
class EDDMootaBankTransfer
{
    ...
    public function register_gateway($gateways)
    {
        ...
        foreach ($banks ?? [] as $bank) {
            ...
            $account = $bank->available_channels;
            if ($bank->bank_type == 'winpayProduction') {
                foreach ($account as $va) {
                    $gateways[$bank->bank_id . "_" . $va->channel_id] = array(
                        'admin_label' => __("Moota VA - {$va->name}", $bank->bank_id),
                        'checkout_label' => __("Moota VA - {$va->name}", $bank->bank_id),
                        'confirmation_label' => __("Moota VA - {$va->name}", $bank->bank_id)
                    );

                    add_action("edd_{$bank->bank_id}_{$va->channel_id}_cc_form", '__return_false');
                    add_action("edd_gateway_{$bank->bank_id}_{$va->channel_id}", array($this, 'process_payment'));
                }
                continue;
            } elseif ($bank->bank_type != 'winpay') {
                $gateways[$bank->bank_id] = array(
                    'admin_label' => __("Moota Bank - {$bank->username} / {$bank->account_number}", $bank->bank_id),
                    'checkout_label' => __("Bank Transfer - {$bank->username} / {$bank->account_number}", $bank->bank_id),
                    'confirmation_label' => __("Bank Transfer - {$bank->username} / {$bank->account_number}", $bank->bank_id)
                );
    ...
    public function process_payment($purchase_data)
    {
        ...
        try {
            ...
            $account = array_filter($banks ?? [], function ($bank) use ($bank_id) {
                return $bank->bank_id == $bank_id;
            });

            if (str_contains($bank_id, "_")) {
                $exploded = explode("_", $bank_id);
            ...
            if (!$errors) {
                ...
                if(count($exploded) == 2){
                    ...
                    return wp_redirect($transaction->data->payment_url);
                }
                ...
                <?php } else { ?>
                    ...
                    <div class="flex flex-col gap-1 text-sm">
                        ...
                        <?php if ($result2->news_code) { ?>
                            <div>
                                Masukan kode <span class="px-2 py-1 bg-green-500 text-white font-bold rounded-md">
                                    <?php echo $result2->news_code ?></span> didalam berita transfer untuk transaksi otomatis!
                            </div>
                        <?php } ?>
                    </div>
                    <div class="py-2">
